...
%
Nesse sentido, o capítulo atual irá levantar as devidas considerações sobre o desenvolvimento prático destas arquiteturas. %ccm <-38


Em específico, a arquitetura física utilizou o mesmo conjunto de computadores, utilizando a mesma topologia.
%
Este tópico será abordado neste capítulo com maior profundidade na Seção~\ref{sec:topologia}. %ccm <-38
%
Em seguida, é relatado o funcionamento do sistema de coleta de informações de recursos, a qual será abordada a Seção~\ref{sec:informacoes}. %ccm <-19, 38
...
\begin{enumerate}
    ...
    \item Protocolos Utilizados: Mostra quais protocolos estes serviços utilizam para troca de mensagens em seu ecossistema. Será mostrado utilizando uma tabela de adjacência, tal qual utilizado para relação entre nós em um Grafo.%ccm <-38 
    \item Processamento ativo interno: Demonstra qual o processamento ativo no serviço independente das requisições de seus microsserviços vizinhos ou cliente. %ccm <-36
\end{enumerate}



Ao final será levantado o funcionamento do cliente das arquiteturas Rudy (Subseção~\ref{rudy}), Salz (Subseção~\ref{salz}) e Willson (Subseção~\ref{willson}). %ccm <-38
%
Nesta etapa será exibido o fluxo de mensagens a partir das ações realizadas pelo cliente. %ccm <-38
%
Nesse sentido este capítulo deve abrangir todos os aspéctos técnicos da aplicação, levando em consideração o seu fluxo de funcionamento e integração entre os microsserviços no ecossistema da arquitetura. %ccm <-38
...
%
O principal objetivo desta topologia é segregar as instâncias na camada de rede, evitando interferências do orquestrador de microsserviços que será utilizado na implantação. %ccm <-38
%
...
%
Segregando em regiões, o orquestrador será forçado a realizar a conexão com as instâncias externas, forçando a utilização da pilha completa do protocolo \ac{tcp} e \ac{udp}.%ccm <-38
...
% \begin{figure}[htb!]
%     \caption{Topologia de rede adotada nos experimentos}
%     \label{fig:topologia}
...
%ccm tentar subfigure:  https://tex.stackexchange.com/questions/37581/latex-figures-side-by-side
...
\begin{table}[htb!]
    ...
    \begin{tabular}{|l|l|l|l|l|l|}
        ...
        \begin{tabular}[c]{@{}l@{}}\textit{client\_one}\\ \textit{client\_two}\end{tabular} & Ubuntu Server 16.04 & 1        & 1 GB    & 192.168.2.*  & 5        \\ \hline
...
%
As instâncias \textit{client\_one} e \textit{client\_two} (Tabela~\ref{tab:instancias}) são responsáveis por executar os clientes a qual realizam o ataque de carga ao serviço.
%
...
%
Por fim, a instância \textit{data\_analisys} é responsável por executar o banco de métricas e o sistema de monitoramento. %ccm fazer como itemize e justificar porque 5 clientes
...
%
Nesse sentido, a configuração inicial de cada instância foi feita de forma automatizada, com um \textit{script} na linguagem \textit{Bash} o qual configura o ambiente com a plataforma \textit{Docker}.
%
...
Para esta solução, a pilha de monitoramento escolhida é o banco de dados para métricas Graphite e o gestor web para visualização Grafana. %ccm colocar a URL das ferramentas como footnote
%
...
%
O Cliente de recurso é utilizado no \textit{game\_service} na qual enviará métricas do consumo de recursos pela instância. %ccm <-38
%
Além deste, o cliente de tempo é responsável por popular dados referente ao tempo que levará para executar cada requisição ao serviço, executado no \textit{game\_client}. %ccm <-38

Os dados serão analisados tanto nos gráficos padrões de monitoramento do grafana, quanto utilizando dos dados brutos, na qual são obtidos através da \ac{api} de render do Graphite. %ccm ,-38
%
Nesse sentido, também será dado a flexibilidade de obter métricas para estudos estatísticos externos a pilha padrão. %ccm <-38
...
Em relação aos protocolos utilizados na arquitetura Rudy, temos dois serviços RPC e dois serviços HTTP. %ccm <1, 14
%
