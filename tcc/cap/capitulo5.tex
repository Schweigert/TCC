...
%
Este ponto é importante para permitir a reproduzão dos experimentos executados.
...
Como exibido na Figura~\ref{fig:network_server}, para o contexto deste trabalho, faz sentido dividir em redes para os seguintes contextos:
...
%
Desta forma, faz-se necessário descrever as suas características próprias.

\subsection{OpenStack}
\ref{sec:open_stack}

O ambiente OpenStack refere-se ao Servidor do paradigma cliente servidor.
%
Tal ambiente foi implantado sobre a nuvem Tchê, na qual encontra-se no Centro de Ciências Tecnológicas da UDESC, em Joinville.

O OpenStack é um orquestrador de núvens computacionais \textit{open source} a qual permite gerenciar recursos de computação, armazenamento e comunicação.
%
Este gerenciamento é provido por \ac{api} com um sistema de autenticação padrão~\cite{open_stack_bib}.
