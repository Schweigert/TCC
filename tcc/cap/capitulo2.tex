...
%
Um exemplo de ambiente com personagens ($P_1$ e $P_2$), objetos ($O_1$) e \ac{npcs} () pode ser visualizado na Figura~\ref{fig:obj_e_npc1}~\cite{albion_online_unite}.
...
%
Um exemplo pode ser visto na Figura~\ref{fig:scene_tree}, retirado do motor gráfico \textit{open source} \textit{Godot}.
...
O módulo de \textit{Network}, implementado dentro de um cliente de jogo \ac{mmorpg}, é responsável por realizar as requisições conforme as ações requiridas pelo jogador ao serviço e aplicar os estados na \textit{Scene Tree} com os objetos da área de interesse~\cite{albion_online_unite}.
%
...
\hline
Autor           & \ac{cpu}   & Memória    & Banda      & Custo      & Latência & Limite de conexões \\ \hline
\cite{1417630}  & $\times$   & $\times$   & \checkmark & $\times$   & $\times$ & $\times$ \\ \hline
\cite{7515686}  & $\times$   & $\times$   & $\times$   & \checkmark & $\times$ & $\times$ \\ \hline
\cite{6374456}  & \checkmark & \checkmark & \checkmark & $\times$   & $\times$ & $\times$ \\ \hline
\end{tabular}
\end{adjustbox}
\end{table}

Nos trabalhos de previsão de carga, não foi descrito um método de replicação automática dos serviços a fim de prover alta disponibilidade de forma automatizada.
%
Também não foi repassado o valor limite de conexões que garantem qualidade de serviço ao usuário final, ou não levam em conta a latência desses serviços com os usuários.
