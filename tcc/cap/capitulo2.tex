...
%
O sistema operacional, hardware ou base de execução da aplicação gráfica define a sua plataforma (\textit{e.g.,} GNU/Linux, MS-Windows, Sony PS4, MS-XBox, web, \textit{etc}.)~\cite{adams_1208533}.
...
\begin{itemize}
  ...
  \item \textbf{Servidor}: O computador, ou o conjunto de computadores, que recebe as requisições do cliente a fim de serem processadas pelo Serviço; e
...
%
O jogo Realm of the Mad God\footnote{Realm of the Mad God: \url{https://www.realmofthemadgod.com/}} é um exemplo na qual não exige autenticação do usuário, entretanto ele não armazena em seu banco de dados o progresso do jogador ou permite ações sociais caso o jogador não efetue a autenticação.
...
%
Nos jogos \ac{mmorpg} é comum a restrição da visão do personagem (Figura~\ref{fig:proximidade}), em certos momentos  pelas características de jogabilidade do gênero \ac{mmorpg} em outros por motivos de desempenho e otimização.
%
...
\begin{itemize}
  ...
  \item O microsserviço deve executar com as dependências mínimas; e
...
%
Como relatado na Subseção~\ref{sec:microsservicos}, uma arquitetura de microsserviços permite múltiplas tecnologias, pois a comunicação entre todos os elementos de um microsserviço será pela rede.
%
...
%
Diferente da arquitetura Rudy(Subseção~\ref{rudy}) a qual prioriza a resolução de requisições, evitando a executar métodos consecutivos da mesma conexão, na arquitetura Salz (Figura~\ref{full_salz}) as chamadas de processo remoto são executadas em ordem \ac{fifo}~\cite{salz_albion}.
%
...
%
Nesse sentido, a melhor escolha é o número de núcleos menos um, escolhido após um teste de \textit{stress}, definido na Subseção~\ref{willson}.
