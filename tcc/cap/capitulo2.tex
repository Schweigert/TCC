...
O conceito de jogos eletrônicos (Seção~\ref{sec:jogos_eletronicos}) é amplamente conhecido fazendo parte da nossa cultura cotidianda, sendo referenciado em filmes e obras literárias.
%
Nesse sentido uma breve definição de jogos eletrônicos se faz necessária a fim de introduzir o conceito do gênero \ac{mmorpg} e a diferença entre outros gêneros (Seção~\ref{sec:arvore_generos}).



Os jogos~\ac{mmorpg} são implementados sobre arquitetados em multiplas camadas, e por esse motivo é necessário descrever o conceito de um jogo~\ac{mmorpg} (Seção~\ref{sec:mmorpg}) e a sua jogabilidade (Seção~\ref{sec:jogabilidade}) para permitir a descrição das arquiteturas dos conceitos de Cliente e Serviço.
%
Nesse sentido, as Seções~\ref{sec:cliente} e~\ref{sec:microsservicos} definem a funcionalidade da integração entre serviço e cliente, popularmente utilizados em aplicações comerciais do segmento de jogos \ac{mmorpg}.
%
Dentro deste contexto, também é definido a funcionalidade de uma microsserviços, dando exemplos de seu funcionamento com o escopo de serviços web.



Por fim, será mostrado trabalhos relacionados (Seção~\ref{sec:similares}) que abordem o tema de arquitetura de jogos \ac{mmorpg} e arquitetura de microsserviços.
