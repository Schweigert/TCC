...
%
A proteção do firewall é aplicada no serviço de pagamento e no ponto de acesso ao serviço, podendo ser visualizada na Figura~\ref{full_rudy_fw}.
...
O modelo de processamento do gerente de mundo, visualizado na Figura~\ref{fig:thread_pool} trabalha com um a padrão \textit{Thread Pool}~\cite{Ringler2014Dec, matthiasrudy2011}, executando a chamada de método remoto de cada jogador em uma fila, onde prioriza executar as chamadas sem repetir a mesma conexão.
%
Dessa forma, cada jogador pode executar somente um método concorrente, sem competir com os demais. Todas as requisições são enfileiradas no buffer de rede do serviço.
%
...
%
Neste serviço, o protocolo \ac{rpc}, tanto o cliente quando o serviço podem invocar métodos remotos, tendo como padrão métodos com retorno vazio~\cite{salz_albion, photon_serialization}.
%
O modelo de processos do serviço de jogo pode ser visualizado na Figura~\ref{salz_thread_model}.


\begin{figure}[htb!]
  \caption{Modelo de processos do serviço de jogo.}
  \label{salz_thread_model}
  \includegraphics[height=5.5cm]{arquiteturas/salz_thread_model.png}
  \centering
...
O microsserviço de jogo executa a lógica do jogo, visível na Figura~\ref{salz_thread_model}, para uma única área em um único processo~\cite{salz_albion}.
%
Esta decisão é dada por conta da interação entre objetos ser complicada de executar, devido a quantia semáforos necessários.
%
Outra facilidade de implementar um modelo com um único processo é de facilitar que objetos não sejam manipulados, quando não for necessário~\cite{albion_online_unite, salz_albion}.
%
...
%
Todos os demais microsserviços operam sobre múltiplos processos, a partir do processo de conexão ao serviço~\cite{salz_albion}.
%
Diferente da arquitetura Rudy~\ref{rudy}, as chamadas de processo remoto são executadas por ordem de chegada~\cite{salz_albion}.
...
A arquitura Willson, exibido na Figura~\ref{willson} mostra um gradual entre a arquitetura Rudy (Figura~\ref{full_rudy}) e a arquitetura Salz (Figura~\ref{full_salz}), propondo uma arquitetura onde divide funcionalidades em outros processos, porém ainda mantem-se conservador para dividir funcionalidades que podem estar juntos ao serviço de jogo~\cite{albion_online_unite, willson}.
%
...
O serviço de jogo da arquitetura Willson é comparada ao serviço similar na Salz~\ref{salz}, entretanto utiliza a técnica de \textit{thread pool} com valor de processos fixo conforme o \textit{hardware} hospedeiro do serviço~\cite{stephenclarkewillson2017}.
%
...
O número de processos paralelos para processamento de requisições é o número de núcleos menos um, visando evitar a troca de contexto com o sistema operacional~\cite{willson}.
%
Essa troca de contexto trás variação na latência da resposta do serviço, a qual não tem controle pelo próprio serviço. Nesse sentido, a melhor escolha é o número de núcleos menos um, escolhido após um teste de \textit{stress}~\ref{willson}.
