...
%
Nesse sentido, espera-se obter somente a interferência das arquiteturas expressa nos valores obtidos. %ccm <-16
...
\begin{enumerate}
  \item Arquitetura Rudy (Subseção~\ref{rudy}); %ccm colocar uma breve descrição da motivação.
  \item Arquitetura Salz (Subseção~\ref{salz}); e
  \item Arquitetura Willson (Subseção~\ref{willson}).
\end{enumerate}
...
O cenário será composto por 5 sub-redes, na qual cada uma será responsável pela operação de uma região do ambiente planejado (Figura~\ref{fig:cenario}). %ccm <-11
...
Após ter definido as características básicas do cenário na qual será executado os experimentos,  faz-se necessário definir as características da simulação dos clientes para os experimentos. %ccm <-11
...
O ambiente da simulação será baseado em matrizes. Cada mapa pode ser visualizado como uma matriz de 100x100 unidades. %ccm Pensar em justificativa para 100x100
%
...
A partir desta sequência de testes, espera-se obter valores suficientes para a análise das arquiteturas de microsserviços específico a jogos \ac{mmorpg}. %ccm <-11 espera-se
