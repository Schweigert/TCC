...
Como exibido na Tabela~\ref{tab:create_account_operation_request_time}, podemos perceber que existe uma diferença abrupta na média de tempo de resposta da \ac{api} e na variância.
%
...
No contexto do atual trabalho podemos assumir a variância como disparidade entre os dados.
%
...
A partir deste contexto, podemos definir que a média de tempo de resposta do serviço, ao criar uma nova conta, segue a seguinte relação:
...
Dessa forma, podemos dizer que para a operação de criar conta, em média, do ponto de vista de tempo de resposta, a arquitetura Willson é superior a arquitetura Salz e Rudy, respectivamente.
%
Também podemos afirmar que a estabilidade desta operação, do ponto de vista de tempo de resposta, também segue a mesma sequência.
...
Seguindo a metodologia da média dos dados obtidos, podemos obter os valores de média, variância, máximo e mínimo.
%
...
A partir da Tabela~\ref{tab:create_character_operation_request}, podemos observar que a média dos valores seguem o comportamento encontrado na operação Criar Conta (Subseção~\ref{sec:op_create_account}). Entretanto é notório a diferença com relação a estabilidade da operação de criação de personagem.
...
A partir desta caracteristica, podemos analisar a diferença na estabilidade entre as arquiteturas Rudy e Salz.
%
