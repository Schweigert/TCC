...
O microsserviço a qual será responsável por receber estas requisições não lida com concorrência conforme a demanda dos jogadores simultâneos.
%
A Figura~\ref{fig:create_account_operation_request_time} torna visivel que o comportamento do tempo de resposta não depende do número de jogadores simultâneos.
...
%
Quanto maior o valor, mais instável é o tempo de resposta de determinada operação ao serviço, entretanto ela mesmo sendo instável tenderá a responder em uma média de tempo.
...
\begin{itemize}
 ...
 \item $\overline{CriarConta_{r}}$: Tempo de resposta médio da operação criar conta na arquitetura Salz;
...
Como visível na Figura~\ref{fig:create_character_operation_request}, o comportamento do tempo de resposta da perspectiva do cliente tem comportamento similar a operação criar conta (Subseção~\ref{sec:op_create_account}).
%
...
Seguindo a metodologia da média dos dados obtidos, pode-se obter os valores de média, variância, máximo e mínimo.
%
...
A partir da Tabela~\ref{tab:create_character_operation_request}, pode-se observar que a média dos valores seguem o comportamento encontrado na operação Criar Conta (Subseção~\ref{sec:op_create_account}). Entretanto é notório a diferença com relação a estabilidade da operação de criação de personagem.

Para realizar esta operação, o serviço realiza diversas validações, em sua grande parte realizando troca de mensagens entre os microsserviços de autenticação e validações de campos.
%
...
A partir desta caracteristica, pode-se analisar a diferença na estabilidade entre as arquiteturas Rudy e Salz.
%
...
Mesmo existindo uma flutuação maior na arquitetura Salz, nota-se que seus pontos máximos de tempo de resposta são inferiores a arquitetura Rudy.
%
...
\begin{itemize}
 ...
 \item $\overline{CriarPersonagem_{s}}$: Tempo de resposta médio da operação criar personagem na arquitetura Salz;
 \item $\overline{CriarPersonagem_{w}}$: Tempo de resposta médio da operação criar personagem na arquitetura Willson;
\end{itemize}

Esta relação também expressa a qualidade de serviço, do ponto de vista de tempo de resposta, entregue ao usuário, haja vista que a variação dos valores entre as arquiteturas Salz e Rudy são relevantes estatisticamente, porém com uma magnitude baixa para utilizar como um argumento válido.
%
Dessa forma, entendemos que a arquitetura que entrega a melhor qualidade de serviço é a Willson, seguido por Salz e Rudy, respectivamente.
...
%
Não pode-se dessa forma dizer que o crescimento é linear.
...
%
Também temos um incremento reduzido, se comparado aos demais quadrantes, quando visualizamos o crescimento do quarto quadrante da arquitetura Willson.
%
...
%
Dessa forma podemos analisar se existe uma flutuação maior comparado aos quadrantes anteriores, mesmo com uma média próxima.
%
...
%
Dessa forma, podemos afirmar que a variação aumenta conforme a carga do serviço aumenta.
%
...
\begin{itemize}
 ...
 \item $\overline{IniciarSessao_{s}}$: Tempo de resposta médio da operação iniciar sessão na arquitetura Salz;
...
\begin{itemize}
 ...
 \item $\overline{InstanciarPersonagem_{s}}$: Tempo de resposta médio da operação instanciar personagem na arquitetura Salz;
...
A Figura~\ref{fig:move_character_request_time} exibe os dados obtidos de tempo de resposta ao movimentar o personagem instanciado no chat durante a variação do número de jogadores simultâneos.
%
...
\begin{itemize}
 ...
 \item $\overline{MoverPersonagem_{s}}$: Tempo de resposta médio da operação mover personagem na arquitetura Salz;
...
\begin{itemize}
 ...
 \item $\overline{EnviarMensagem_{s}}$: Tempo de resposta médio da operação enviar mensagem na arquitetura Salz;
...
\begin{itemize}
 ...
 \item $\overline{ReceberMensagem_{s}}$: Tempo de resposta médio da operação receber mensagem na arquitetura Salz;
...
%
Para este caso pode-se analisar que a curva estabiliza o seu crescimento entre 20 e 40 jogadores simultâneos, a mesma faixa onde os bancos de dados estabilizaram seu crescimento (visível na Figura~\ref{fig:cpu_db_media_por_jogador}).
%
...
A partir da Tabela~\ref{tab:mem_db_media_quadrante}, pode-se mensurar a distância de consumo de memória por parte do PostgreSQL.
%
...
%
Dado tais informações pode-se parcialmente concluir que o consumo de memória por parte do banco de dados persistentes é menor na arquitetura Rudy, entretanto o consumo é demasiadamente próximo entre as arquiteturas Willson e Salz, tendo a arquitetura Salz uma leve tendência a ser maior.
