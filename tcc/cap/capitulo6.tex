...
%
Sendo assim, faz sentido agrupar tais experimentos, que estão agrupados na Seção~\ref{sec:experimentos}, para posterior comparação. %cccm <-16
...
%
Entretanto, nota-se que todos os microsserviços contém uma característica senoidal, na qual sua amplitude aumenta com a quantidade de jogadores, na qual já era visível na Figura~\ref{fig:experimento_game_cpu}.
%
...
Foi obtido comparações ao realizar uma análise individual dos critérios obtidos nos experimentos.
%
...
%
A partir destes dados, podemos tomar algumas conclusões.
%
Dessa forma, conclui-se:

\begin{itemize}
 \item \textbf{Operação de Sessão} é uma categoria de requisições na qual não compartilha características com outras operações;
 ...
 \item \textbf{Saída de Rede dos microsserviços} é relaciona-se  as requisições da operação de mundo, visto que tais operações possuem alta frequência de requisições.
\end{itemize}
...
\begin{itemize}
 \item Operações Web: A arquitetura Willson mostrou o menor tempo de resposta por uma melhor ocupação de \ac{cpu} e conexão direta ao banco de dados;
 \item Operação de Sessão: A arquitetura Salz mostrou o menor tempo de resposta por fornecer o microsserviço de autenticação de forma pública para conexão pelo cliente, diminuindo a cadeia de conexões necessárias para realizar a autenticação no serviço;
 \item Operações de Mundo: A arquitetura Willson mostrou o menor tempo de resposta por mostrar um melhor aproveitamento de \ac{cpu};
 \item Operações do Mundo: A arquitetura Willson mostrou o menor tempo de resposta por mostrar um melhor aproveitamento de \ac{cpu}; e
 \item Operações de Chat: A arquitetura Willson mostrou o menor tempo de resposta por mostrar um melhor aproveitamento de \ac{cpu};
\end{itemize}
...
%
Tal resultado é proveniente do enfileiramento de requisições pelo microsserviço \textit{rcrud}, a qual obtrui a comunicação com o banco de dados, tornando uma comunicação serial.

Para a arquitetura de microsserviços, a arquitetura Salz obteve o menor consumo de \ac{cpu} em média.
%
Este resultado é proveniente da divisão dos microsserviços \textit{sgame} e \textit{schat}, na qual obstruem com comunicação interna a \ac{cpu}.
...
%
Isto é um reflexo direto, positivo, da minimização de requisições simultâneas a um serviço de fonte de dados.


Para o consumo de memória por parte do serviço a arquitetura Willson mostrou-se a mais econômica.
%
Esta economia de memória é proveniente da minimização de microsserviços, e consecutivamente processos concorrentes para execução das mesmas tarefas.
%
Entretanto, este ponto não necessáriamente é positivo, mas sim uma característica do experimento.
Nota-se que no caso de necessidade de escalar a arquitetura Willson, será necessário escalar sistemas que talvez sejam desnecessários para a operação do serviço.

Para o trafego de rede, notou-se que a entrada e saída de dados no banco de dados tendem a crescer com a mesma intencidade.
%
...
%
Isto é um reflexo direto ao enfileiriamento causado pelo microsserviço \textit{rcrud}.
...
%
Esta característica é dada pelo elevado consumo de rede interna para realizar consultas ao banco de dados.
